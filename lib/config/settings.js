...
settings[`${consts.CF_MANUAL_SUGGEST}`] = {
    type: 'boolean',
    description:
        'if activated (**recomended**), suggestions are shown by pressing `ctrl` + `space` shortcut. Uncheck to get suggestions at typing.',
    default: false
};
settings[`${consts.CF_ALLOWED_SCOPES}`] = {
    ...
    default:
        '.source .string.quoted, .source.shell, .text .string, .text.html.basic'
};
settings[`${consts.CF_PROVIDER_STRATEGY_ALL}`] = {
    type: 'boolean',
    description:
        'If activated. all providers that can resolve suggestions are invoked (A bit lower operation). Disabled by default.',
    default: false
};
settings[`${consts.CF_ENABLE_DEBUG}`] = {
    type: 'boolean',
    description:
        "If activated, enables debug options for this package. Note that Atom's dev mode `$ atom --dev .` overrides this setting.",
    default: 'false'
};
