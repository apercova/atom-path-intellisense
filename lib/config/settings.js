...
settings[`${consts.CF_MANUAL_SUGGEST}`] = {
    type: 'boolean',
    description:
        'if enabled (**recomended**), suggestions are shown by pressing `ctrl` + `space` shortcut. Uncheck to get suggestions at typing. Enabled by default.',
    default: true
};
...
settings[`${consts.CF_ENABLE_DEBUG}`] = {
    type: 'boolean',
    description:
        "If activated, it enables debug options for this package. Note that Atom's dev mode `$ atom --dev .` overrides this setting.",
    default: 'false'
};
