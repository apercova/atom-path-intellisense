...
//const PATTERN_PATH_REGEX = /(((\/|\.\/|\.\.\/)([^\/]+\/)*)([^\/]+)?)$/;
//const REL_PATH_REGEX = /[\"\']\s*(?!\/|\.\/|\.\.\/)(([^\/]+\/)*([^\/]+)?)$/;
const PATH_WIN_REGEX = /(((\/|\.\/|\.\.\/)([^\\\/\:\*\?\"\<\>\|]+\/)*)([^\\\/\:\*\?\"\<\>\|]+)?)$/;
//const NO_PAT_WIN_REGEX = /[\"\']\s*((([^\\\/\:\*\?\"\<\>\|]+\/)*)([^\\\/\:\*\?\"\<\>\|]+))$/;
...
const _fn = {
    ...
    const promize = new Promise(function(resolve, reject){
      ...
      if (filepath) {
        ...
        .then(function(searchpath){
          const suggestions = [];
          suggestions.push(self.formatDirectory(req, "../", filepath.file));
          fs.readdir(searchpath, function(e, entries){
            entries.forEach(function (entry) {
              entryStats = fs.statSync(path.resolve(searchpath, entry));
              if (entryStats) {
                if(entryStats.isDirectory()){
                  suggestions.push(self.formatDirectory(req,`${entry}/`, filepath.file));
                }
                if(entryStats.isFile()){
                  suggestions.push(self.formatFile(req, entry, filepath.file));
                }
              }
            });
  ...
  getSuggestions: function(req){
    ...
    line = this.unescapeStringQuote(line);
    console.dir(line);
    console.dir(parser.patterns.pathregex);
...
DefaultPathProvider.prototype.loadConfig = function(req){
