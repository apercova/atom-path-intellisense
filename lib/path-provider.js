...
const fn = {
  ...
  resolvePathSuggestions: function(req, parsedPath) {
    ...
    const promize = new Promise(function(resolve){
      ...
      .then(function(searchPath){
        //Fixed parent dir
        suggestions.push(self.formatDirectory("../", req, parsedPath));
          ...
          try {
            ...
            if (entryStats) {
              if(entryStats.isDirectory()){
                suggestions.push(self.formatDirectory(`${entry}/`, req, parsedPath));
              }
              if(entryStats.isFile()){
                suggestions.push(self.formatFile(entry, req, parsedPath));
              }
            }
          }
          catch(e) {
  ...
  resolveSearchPath: function(req, cursorPath){
    ...
    const promize = new Promise(function(resolve, reject){
      try{
        ...
        try {
          const basePath = path.parse(path.parse(req.editor.getPath()).dir);
  ...
  formatFile:function(file, req, parsedPath){
    return {
      ...
      replacementPrefix: this.escapeStringQuote(this.getReplacementPrefix(req, parsedPath)||''),
      type:"tag",
