...
class AtomPathIntellisenseImpl {
    ...
    activate() {
        ...
        return new Promise((resolve, reject) => {
            self._registerFormatters()
                .then(formatters => {
                    self._logger.debug(
                        `${formatters.length} formatters registered.`
                    );
                    self._registerPathProviders()
                        ...
                        .catch(e => {
                            self._logger.error(
                                'Error activating auto-complete provider.'
                            );
                            reject(e);
                        });
                })
    ...
    dispose() {
        const self = this;
        return Promise.all([self._disposePathProviders()]);
    }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                self.pathProviders.push(
                    new CurrentFilePathProvider(
                        this.formatters.find(f => f.id === 'DefaultFormatter')
                    )
                );
                self.pathProviders.push(
                    new CurrentFileRelativePathProvider(
                        this.formatters.find(f => f.id === 'DefaultFormatter')
                    )
                );
                self.pathProviders.push(
