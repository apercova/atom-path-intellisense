...
import { parse, selectorMatchesAnyScope, matcherForSelector, isSubsetX } from './util/selectors.js';
...
export default class AtomPathIntellisenseImpl {
  ...
  _testSelectors(req) {
    const _scopes = req.editor.getLastCursor().getScopeDescriptor().scopes;
    console.log(_scopes);
    const _scopeChain = req.editor.getLastCursor().getScopeDescriptor().getScopeChain();
    console.log(_scopeChain);
    const sup = ["source", "Volvo", "BMW"];
    const sub = ["Saab", "Volvo", "BMW"];
    let subset = isSubset(sub, sup);
    console.log(`isSubset: ${subset}`);

    const selector = 'source.js .string.quoted';
    const scopes = req.editor.getLastCursor().getScopeDescriptor().scopes;
    console.dir(scopes);
    const sup2 = parse(selector);
    console.dir(sup2);
    const sub2 = parse(scopes[0]);
    console.dir(sub2);
    let subset2 = isSubsetX(sub2, sup2);
    console.log(`isSubset2: ${subset2}`);
    let subset3 = isSubsetX(sub2, sup2);
    console.log(`isSubset21: ${subset3}`);
  }
  _testSelectors2(req) {
    const selector = 'source.js .string.quoted';
    ...
    let partsSelector = parse(selector);
    console.log('partsSelector: ');
    console.dir(partsSelector);

    let partsScope = parse(scopes[0]);
    console.log('partsScope: ');
    console.dir(partsScope);

    let subset = isSubset(partsScope, partsSelector);
    console.log(`isSubset: ${subset}`);
