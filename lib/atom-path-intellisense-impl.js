...
class AtomPathIntellisenseImpl {
  ...
  _providerFilter(req) {
    ...
    const cfScopeSelector = config.getAllowedScopes();
  ...
  _resolveBestPathProvider(req) {
    return this.pathProviders.sort((a, b) => a.priority - b.priority).find(this._providerFilter(req));
  }
