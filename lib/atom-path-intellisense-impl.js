...
class AtomPathIntellisenseImpl {
                    ...
                    self._registerFormatters()
                        .then(() => {
                            self._registerPathProviders()
                                .then(() => {
                                    self._logger.debug(
                                        'Auto-complete provider activated.'
                                    );
                                    resolve(true);
                                })
                                .catch(e => {
                                    self._logger.error(
                                        'Error registering path providers.'
                                    );
                                    reject(e);
                                });
                        })
                        .catch(e => {
                            self._logger.error('Error registering formatters.');
                            reject(e);
                        });
                })
                .catch(e => {
                    self._logger.error(
                        'Error registering configuration observers.'
                    );
                    reject(e);
                });
        });
    ...
    _resolveSuggestionsAcurracyAll(req) {
        ...
        return new Promise((resolve, reject) => {
            try {
                self._logger.debug(
                    'Using all providers strategy. Priority does not matter.'
                );
    ...
    _resolveSuggestionsAccuracyBest(req) {
        ...
        return new Promise((resolve, reject) => {
            try {
                self._logger.debug(
                    'Using priority-based best provider strategy.'
                );
                const pathProvider = this._resolveBestPathProvider(req);
                if (pathProvider) {
                    self._logger.debug(
                        `Best provider found: ${pathProvider.id}`
                    );
                    pathProvider
                        .resolve(req)
                ...
                self._logger.error(
                    'Error using priority-based best provider strategy.'
                );
                reject(e);
            }
        });
    ...
    /**
     * _registerFormatters - Register formatters used by providers.
     *
     * @return {void}
     */
    _registerFormatters() {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                self._logger.debug(
                    `${self.formatters.length} formatters registered.`
                );
    ...
    _registerPathProviders() {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                self.pathProviders.push(
                    new NodeJSPathProvider(
                        this.formatters.find(f => f.id === 'NodeJSFormatter')
                    )
                );
                self._logger.debug(
                    `${self.pathProviders.length} path providers registered`
                );
                ...
                Promise.all(self.pathProviders.map(p => p.activate()))
                    ...
                    .catch(e => {
                        ...
                        if (_provider) {
                            self._logger.debug(
                                `Deactivating provider ${_provider.id}.`
                            );
                            self.pathProviders = self.pathProviders.filter(
                                p => p.id !== _provider.id
                            );
                            self._logger.debug(
                                `Provider ${_provider.id} deactivated.`
                            );
                        }
                        resolve(self.pathProviders);
                    });
    ...
    _resolveAllPathProviders(req) {
        const scopesAtCursor = req.editor.getLastCursor().getScopeDescriptor()
            .scopes;
        this._logger.debug(`scopesAtCursor: ${scopesAtCursor}`);
        const cfScopeSelector = atom.config.get(
            `${consts.PACKAGE_NAME}.${consts.CF_ALLOWED_SCOPES}`
        );
        this._logger.debug(`cfScopeSelector: ${cfScopeSelector}`);
        const providers = this.pathProviders.filter(p => {
            let result =
                p instanceof BasePathProvider
                    ? p.id === CurrentFilePathProvider.id ||
                      p.id === CurrentFileRelativePathProvider.id
                        ? selectors.anySelectorMatchAllScopes(
                            cfScopeSelector || p.scopeSelector || '',
                            scopesAtCursor
                        )
                        : selectors.anySelectorMatchAllScopes(
                            p.scopeSelector || '',
        ...
        return new Promise((resolve, reject) => {
            try {
                Promise.all(self.pathProviders.map(p => p.dispose()))
                    .then(providers => {
                        self._logger.debug(
                            `${providers.length} providers disposed.`
                        );
                        providers.forEach(_provider => {
                            self._logger.debug(
                                `Provider ${_provider.id} disposed.`
                            );
                        });
                        resolve(self.pathProviders);
                    })
                    .catch(e => {
                        ...
                        if (_provider) {
                            self._logger.warn(
                                `Provider ${_provider.id} not disposed.`
                            );
                        }
                        resolve(self.pathProviders);
                    });
