...
class AtomPathIntellisenseImpl {
  ...
  dispose() {
    const self = this;
    self._logger.debug("Disposing auto-complete provider.");
    return [];
  }
  _disposePathProviders() {
    const self = this;
    return new Promise((resolve, reject) => {
        self._logger.debug(`Disposing providers.`);
        try {
          Promise.all(this.pathProviders.map(p => p.dispose()))
          .then(values => {
            console.error('mmmmmmmmmmmm');
            self._logger.debug(`Providers activated.`);
            self._logger.debug(`Providers registered.`);
            console.log(self.pathProviders);
          ...
          .catch(e => {
            ...
            if (_provider) {
              self._logger.debug(`Provider not disposed: ${_provider.id}.`);
            }
            resolve(self.pathProviders);
          });
        } catch (e) {
          self._logger.debug(`Error disposing providers.`);
          reject(e);
        }
    });
