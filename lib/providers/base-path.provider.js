...
class BasePathProvider {
    ...
    activate() {
        ...
        /*return Promise.reject({
			error: new Error('Not implemented in base class!2'),
			provider: this
		});*/
    }
                        ...
                        if (Array.isArray(entries)) {
                            ...
                            entries.forEach(entry => {
                                try {
                                    let entryStats = fs.statSync(
                                        path.resolve(result.searchPath, entry)
                                    );
                                    if (entryStats) {
                            ...
                            if (Array.isArray(self.fileExtFilter)) {
                                ...
                                result.suggestions = result.suggestions.filter(
                                    s => {
                                        return s.type === 'file'
                                            ? self.fileExtFilter.some(
                                                ext =>
                                                    ext ===
                                                      path.parse(s.entry).ext
                                            )
                                            : true;
                                    }
                                );
    ...
    $getCurrentLine(req, unescape) {
        let line = req
            ? stringUtils.unescapeStringQuote(
                req.editor.getLastCursor().getCurrentBufferLine()
            )
            : '';
        line = unescape ? stringUtils.unescapeStringQuote(line) : line;
        line = line
            ? unescape
                ? stringUtils.unescapeStringQuote(line)
                : line
            : line;
    ...
    $getCurrentFilePath(req, unescape) {
        ...
        filePath = filePath
            ? unescape
                ? stringUtils.unescapeStringQuote(filePath)
                : filePath
            : filePath;
