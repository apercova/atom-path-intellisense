...
import SearchPathDTO from '../dto/suggestions.dto.js';
  ...
  $resolveSuggestions(req, search) {
    ...
    const promize = new Promise((resolve, reject) => {
      ...
      if (!search instanceof SearchPathDTO) {
        ...
        try {
					fs.readdir(result.searchPath, (e, entries) => {
						result.suggestions.push({type: 'dir', entry: '..'});
						entries.forEach((entry) => {
							entryStats = fs.statSync(path.resolve(result.searchPath, entry));
							if (entryStats) {
            ...
            if (Array.isArray(self.fileExtFilter)) {
              console.dir(result.suggestions);
              console.log('Filtering files by extension');
