...
class LessPathProvider extends BasePathProvider {
  ...
  constructor() {
    ...
    this.priority = 9991;
    this.scopeSelector = '.source.css.less .meta.at-rule.import.css';
    this.formatter = new DefaultFormatter({ trimext: true });
    this.fileExtFilter = ['.css', '.less'];
    this._logger = logger.getLogger(this.id);
    this._testPathRegStr =
      '@import\\s*?(?:\\((?:reference|once)\\))?\\s*?["\'](.*)["\']?' +
      '|@import\\s*(?:\\((?:reference|once)\\))?\\s*(.*)';
    this._testPathRegex = new RegExp(this._testPathRegStr);
  }
   ...
   */
  canResolve(req) {
    return this.$getCurrentLineUpCursor(req, true).match(this._testPathRegex) ? true : false;
  }
   ...
   */
  resolve(req) {
    const line = this.$getCurrentLineUpCursor(req, true);
  ...
  $getTestPath(req, line) {
    ...
    let testPath = match ? match[1] || match[2] || '' : '';
