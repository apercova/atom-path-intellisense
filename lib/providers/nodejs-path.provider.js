...
class NodeJSPathProvider extends BasePathProvider {
    constructor(formatter) {
        ...
        this.requirePatternRegex = /require\(['"](.*?)['"]\)/;
        this.importPatternRegex = /import.*?from.*?['"](.*?)['"]/;
        this.prefixRegex = /([^/]+?)$/;
        this.biModRegex = /^[^~./].*$/;
            ...
            try {
                ...
                exec('npm root -g', (err, stdout, stderr) => {
                    if (err) {
                        ...
                        self.global_node_modules = path.format(
                            path.parse(`${stdout}`.trim())
                        );
                        self._logger.debug(
                            `Global node_modules path: ${self.global_node_modules}`
                        );
                        resolve(this);
                    }
                });
    ...
    resolve(req) {
        ...
        return new Promise((resolve, reject) => {
            self.$resolveSuggestions(req, self.$resolveSearchPathSync(req))
                .then(suggestions => {
                    if (self._getTestPath(req).match(self.biModRegex)) {
    ...
    _getTestPath(req) {
        ...
        let testPath = '';
        let match = line.match(this.requirePatternRegex);
        if (match) {
            testPath = match[1];
        }
        match = line.match(this.importPatternRegex);
        if (match) {
            testPath = match[1];
        }
        return testPath;
