...
export default class CurrentFileRelativePathProvider extends BasePathProvider {
  ...
  canResolve(req) {
		return this.$getCurrentLineUpCursor(req).match(this._relPathRegex) ? true : false;
	}
  resolve(req) {
    const self = this;
    const promize = new Promise((resolve, reject) => {
      ...
      const search = self._resolveValidPathSync(
        self, req,
        self.$getCurrentLineUpCursor(req)
      );
      console.dir(search);
      ...
      // resolve(result);
        ...
        if (search && search.searchPath) {
          result = new SuggestionsDTO(self.$getPrefix(req, search), []);
          try {
