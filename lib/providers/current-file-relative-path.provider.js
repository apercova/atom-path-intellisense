   ...
   */
   _resolveValidPathSync($self, req, testPath, searchPath, basePath, idx) {
     ...
     if (!matches || !matches[0]) {
       return $self._resolveValidPathSync($self, req, undefined, undefined, undefined, idx);
     } else {
         ...
         try {
           ...
           if (!stats.isDirectory()) {
             ...
             return $self._resolveValidPathSync($self, req, testPath, fullTestPath, basePath, idx);
           } else {
         ...
         } catch(e)
         {
           ...
           if (!testPath.endsWith('/')) {
               ...
               try {
                 ...
                 return $self._resolveValidPathSync($self, req, testPath, undefined, basePath, idx);
               }
           } else {
    ...
    const self = this;
		return new Promise((resolve, reject) => {
			try {
				...
				this.formatter = null;
				console.log(`Path provider: ${this.id} disposed.`);
			} catch(e) {
				console.warn(`Error disposing path provider: ${this.id}`);
				reject(e);
			}
		});
    /*return Promise.reject({
			error: new Error('Not disposable!!!'),
			provider: this
		});*/
    //this._logger.debug(`Disposing path provider: ${this.id}`);
		//winston.loggers.close(this.id);
