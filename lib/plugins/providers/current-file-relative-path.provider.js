...
export default class CurrentFileRelativePathProvider extends BasePathProvider {
  ...
  resolve(req) {
    ...
    const promize = new Promise((resolve, reject) => {
      ...
      const search = self.$resolveValidPathSync(
        self, req,
      ...
      if (!search) {
        ...
        if (search && search.searchPath) {
          result = new SuggestionsDTO(
      			this.$getPrefix(req, search),
      			[]
      		);
          try {
  ...
  $resolveValidPathSync($self, req, testPath, foundPath, idx) {
    let basePath = $self.$getCurrentFilePath(req);
    console.dir(`basepath: ${basePath}`);
    // foundPath = foundPath ? foundPath : false;
    idx = !isNaN(idx) ? idx : 0;
    idx ++;
    if (foundPath) {
      return {
      ...
      if (!matches || !matches[0]) {
        return $self.$resolveValidPathSync($self, req, undefined, undefined, idx);
      } else {
        testPath = testPath.substring(matches.index);
        console.warn(`trying ${testPath}`);
        if(testPath.startsWith('/')) {
          // Fix for project relative dir
        ...
        try {
          ...
          if (!stats.isDirectory()) {
            ...
            return $self.$resolveValidPathSync($self, req, testPath, fullTestPath, idx);
          } else {
        ...
        } catch(e)
        {
          ...
          if (!testPath.endsWith('/')) {
              ...
              try {
                ...
                return $self.$resolveValidPathSync($self, req, testPath, undefined, idx);
              }
          } else {
