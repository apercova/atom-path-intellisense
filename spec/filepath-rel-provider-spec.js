...
const AtomPathIntellisenseImpl = require('../lib/atom-path-intellisense-impl');
...
describe(consts['PACKAGE_NAME'], () => {
  ...
  beforeEach(() => {
    ...
    waitsForPromise(() =>
      Promise.all([
        atom.workspace.open(path.join(process.cwd(),'test','files','sample.js'))
          .then((e) => editor = e),
  ...
  it('can resolve filepath suggestions', () => {
    editor.setText('import fs from \'\';');
    editor.setCursorBufferPosition([0, 16]);
    expect(editor.getTextInBufferRange([[0,0],editor.getLastCursor().getBufferPosition()])).toEqual('import fs from \'');

    waitsForPromise(() => getSuggestions(provider, editor)
      .then((suggestions) => {
  ...
  it('can resolve filepath-relative suggestions (../esca\'ped_dir/)', () => {
    ...
    expect(editor.getTextInBufferRange([[0,0],editor.getLastCursor().getBufferPosition()])).toEqual('import fs from (\'../esca\\\'ped_dir/');
  ...
  it('can resolve on xml files', () => {
    let xmlEditor;
    waitsForPromise(() =>
      atom.workspace.open(path.join(process.cwd(),'test','files','sample.xml'))
      .then((e) => {
        xmlEditor = e;
        xmlEditor.setText('<root path="./"></root>');
        xmlEditor.setCursorBufferPosition([0, 14]);
        expect(xmlEditor.getTextInBufferRange([[0,0],xmlEditor.getLastCursor().getBufferPosition()])).toEqual('<root path="./');
    ...
    waitsForPromise(() => getSuggestions(provider, xmlEditor)
      .then((suggestions) => {
        expect(suggestions).toBeInstanceOf(Array);
        expect(suggestions).toHaveLength(3);
        expect(suggestions[0].displayText).toEqual('../');
