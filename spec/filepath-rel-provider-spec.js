...
describe(consts['PACKAGE_NAME'], () => {
    ...
    beforeEach(() => {
        ...
        runs(() => {
            provider = atom.packages
                .getActivePackage('atom-path-intellisense')
    ...
    it('can resolve scopes at cursor', () => {
        ...
        let validScopes = selectors.selectorMatchesAllScopes(
    ...
    it("can resolve filepath-relative suggestions (../esca'ped_dir/)", () => {
        ...
        ).toEqual("import fs from ('../esca\\'ped_dir/");
        ...
        waitsForPromise(() =>
            getSuggestions(provider, editor).then(suggestions => {
                ...
                expect(suggestions).toHaveLength(3);
                expect(suggestions[0].displayText).toEqual('../');
                expect(suggestions[1].displayText).toEqual(
                    "esca'ped_file01.js"
                );
                expect(suggestions[2].displayText).toEqual(
                    "esca'ped_file02.js"
                );
            })
        );
        ...
        waitsForPromise(() =>
            atom.workspace
                ...
                .then(e => {
                    ...
                    expect(
                        xmlEditor.getTextInBufferRange([
                            [0, 0],
                            xmlEditor.getLastCursor().getBufferPosition()
                        ])
                    ).toEqual('<root path="./');

                    let scopesAtCursor = xmlEditor
                        .getLastCursor()
                        .getScopeDescriptor().scopes;
                    expect(scopesAtCursor).toBeInstanceOf(Array);
                    expect(scopesAtCursor).toHaveLength(4);
                    let validScopes = selectors.selectorMatchesAnyScope(
                        '.text.xml .string.quoted.double',
                        scopesAtCursor
                    );
                    expect(validScopes).toEqual(true);
                })
